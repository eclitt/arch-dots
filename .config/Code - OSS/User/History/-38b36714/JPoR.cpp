...
int main() {
    ...
    return 0;
} // setf | убрать даты отчисление если учаться| шаблоны по фамилии поиск для удаления / (отчислен убрать) / то есть оставить дату отчисления если отчислен / поиск по шаблону и фамилия И группа / редактирование (поиск оп различным полям) / вставка с сохранением порядка (выбор идеального места) \ вывод определенной записи / 